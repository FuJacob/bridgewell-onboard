...
export async function saveTemplate(
  templateName: string,
  questions: Question[],
  _templateFiles?: { [key: string]: File }
): Promise<SaveTemplateResponse> {
  console.log("=== DEBUG: saveTemplate service called ===");
  console.log("templateName:", templateName);
  console.log("questions:", questions);
  console.log("templateFiles ignored for template save");

  // Always send JSON without files. Strip per-question templates before sending.
  const questionsWithoutTemplates = (questions || []).map((q, idx) => ({
    ...q,
  ...
  const response = await fetch("/api/admin/save-template", {
    ...
    body: JSON.stringify({ templateName, questions: questionsWithoutTemplates }),
  });
